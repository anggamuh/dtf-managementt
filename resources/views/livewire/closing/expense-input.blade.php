<div>
    <div class="bg-white rounded-lg shadow p-6 dark:bg-slate-800 dark:text-slate-200">
            ...
            <table class="min-w-full divide-y divide-gray-200 text-sm">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-3 py-2 text-left">No Bon</th>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-slate-700">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-1">
                            <input type="text" wire:model="rows.{{ $index }}.bon_number" class="w-16 border-gray-300 rounded text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" placeholder="#">
                        </td>
                        <td class="px-3 py-1">
                            <input type="text" wire:model="rows.{{ $index }}.category" class="w-28 border-gray-300 rounded text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" placeholder="Kategori">
                            @error("rows.{{ $index }}.category") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </td>
                        <td class="px-3 py-1">
                            <input type="text" wire:model="rows.{{ $index }}.description" class="w-full border-gray-300 rounded text-sm dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100" placeholder="Rincian">
                            @error("rows.{{ $index }}.description") <span class="text-red-500 text-xs">{{ $message }}</span> @enderror
                        </td>
                        <td class="px-3 py-1">
                            <input type="number" step="1" wire:model="rows.{{ $index }}.qty" class="w-16 border-gray-300 rounded text-sm text-right dark:border-slate-700 dark:bg-slate-800 dark:text-slate-100">
                        </td>
                        <td class="px-3 py-1">
        ...
        <div class="flex items-center justify-between">
            ...
            <button wire:click="save" class="branch-required inline-flex items-center px-4 py-2 bg-red-600 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-red-700">
                Simpan Pengeluaran
            </button>
        </div>
    </div>
