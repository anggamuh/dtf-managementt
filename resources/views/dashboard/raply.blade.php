...
@section('title', 'Dashboard RAPLY')

@section('content')
<div class="space-y-6">
    <div class="flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Dashboard RAPLY</h1>
        <span class="text-sm bg-blue-100 text-blue-800 px-3 py-1 rounded-full">Cabang RAPLY</span>
    </div>
    ...
    <div class="bg-white rounded-lg shadow p-6 dark:bg-slate-800 dark:text-slate-200">
        <h3 class="text-lg font-medium text-gray-900 mb-4 dark:text-slate-100">Data Pendapatan (Pemasukan)</h3>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-slate-700">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-center">
                            <form method="POST" action="{{ route('invoices.destroy', $invoice) }}" onsubmit="return confirm('Hapus?')">
                                @csrf @method('DELETE')
                    ...
                    @empty
                    <tr><td colspan="6" class="text-center text-gray-500 py-4">Belum ada data pendapatan</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
    ...
    <!-- Data Pengeluaran -->
    <div class="bg-white rounded-lg shadow p-6 dark:bg-slate-800 dark:text-slate-200">
        <h3 class="text-lg font-medium text-gray-900 mb-4 dark:text-slate-100">Data Pengeluaran</h3>
        <div class="overflow-x-auto">
            <table class="min-w-full divide-y divide-gray-200 text-sm dark:divide-slate-700">
                <thead class="bg-gray-50 dark:bg-slate-900">
                    <tr>
                ...
                <tbody class="bg-white divide-y divide-gray-200 dark:bg-slate-800 dark:divide-slate-700">
                    ...
                    <tr>
                        ...
                        <td class="px-3 py-2 text-center">
                            <form method="POST" action="{{ route('expenses.destroy', $expense) }}" onsubmit="return confirm('Hapus?')">
                                @csrf @method('DELETE')
                                <button type="submit" class="text-red-600 hover:text-red-900 text-xs">Hapus</button>
                            </form>
                        </td>
                    ...
                    @empty
                    <tr><td colspan="5" class="text-center text-gray-500 py-4">Belum ada data pengeluaran</td></tr>
                    @endforelse
                </tbody>
            </table>
        </div>
